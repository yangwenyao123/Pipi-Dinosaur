...
export default {
  ...
  render(h) {
    const { $slots, count, activeIndicator } = this;
    
    const Indicator =
      $slots.indicator ||
      (this.showIndicators && count > 1 && (
        <div class={['swipe__indicators', this.vertical === true? 'swipe__indicators--vertical':'']}>
          {Array(...Array(count)).map((empty, index) => (
            <i
              class={['swipe__indicator', { active: index === activeIndicator }]}
